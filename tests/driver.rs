...
use retty_io::{buf::IoBuf, fs::File};
...
fn complete_ops_on_drop() {
    ...
    unsafe impl retty_io::buf::IoBufMut for MyBuf {
...
fn completion_overflow() {
    ...
    retty_io::builder()
        ...
        .uring_builder(retty_io::uring_builder().setup_cqsize(cqueue_entries))
        .start(async move {
            let mut js = JoinSet::new();

            for _ in 0..spawn_cnt {
                js.spawn_local(retty_io::no_op());
            }
