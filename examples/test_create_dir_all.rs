...
async fn statx<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let _statx = retty_io::fs::statx(path).await?;
    Ok(())
}

async fn statx_builder<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let _statx = retty_io::fs::StatxBuilder::new()
        .pathname(path)?
        .statx()
        .await?;
    Ok(())
...
async fn statx_builder2<P: AsRef<Path>>(dir_path: P, rel_path: P) -> io::Result<()> {
    ...
    // descriptor.
    let f = retty_io::fs::File::open(dir_path).await?;
...
async fn matches_mode<P: AsRef<Path>>(path: P, want_mode: u16) -> io::Result<()> {
    let statx = retty_io::fs::StatxBuilder::new()
...
async fn touch_file<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let file = retty_io::fs::OpenOptions::new()
        .append(true)
        .create(true)
        .open(path)
        .await?;
...
async fn is_regfile<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let (_is_dir, is_regfile) = retty_io::fs::is_dir_regfile(path).await;
...
async fn is_dir<P: AsRef<Path>>(path: P) -> io::Result<()> {
    let (is_dir, _is_regfile) = retty_io::fs::is_dir_regfile(path).await;
