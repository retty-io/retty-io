...
fn parse_knobs(
    ...
    let mut rt = match config.driver {
        DriverType::Legacy => {
            quote_spanned! {last_stmt_start_span=>retty_io::RuntimeBuilder::<retty_io::LegacyDriver>::new()}
        }
        DriverType::Uring => {
            quote_spanned! {last_stmt_start_span=>retty_io::RuntimeBuilder::<retty_io::IoUringDriver>::new()}
        }
        DriverType::Fusion => {
            quote_spanned! {last_stmt_start_span=>retty_io::RuntimeBuilder::<retty_io::FusionDriver>::new()}
        }
    };
