...
#[cfg(unix)]
#[retty_io::test_all(timer_enabled = true)]
async fn connect_timeout_dst() {
    ...
    let drop_flag_copy = drop_flag.clone();
    {
        ...
        let res = retty_io::select! {
            _ = connect => { false }
            _ = retty_io::time::sleep(std::time::Duration::from_secs(1)) => { true }
        };
        assert!(res);
    }
...
async fn cancel_read() {
    use retty_io::io::CancelableAsyncReadRent;

    let mut s = TcpStream::connect("rsproxy.cn:80").await.unwrap();
    let buf = vec![0; 20];

    let canceler = retty_io::io::Canceller::new();
    let handle = canceler.handle();
    retty_io::spawn(async move {
        retty_io::time::sleep(std::time::Duration::from_millis(100)).await;
        canceler.cancel();
    });
...
async fn cancel_select() {
    ...
    let timer = retty_io::time::sleep(std::time::Duration::from_millis(100));
    retty_io::pin!(timer);
