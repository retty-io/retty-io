...
/// allocated in memory, that can be registered in the current `retty-io`
...
///   in flight;
/// - The `retty-io` [`Runtime`] the buffers are registered with
///   has been dropped.
///
...
///
/// ```ignore
/// use retty_io::buf::fixed::FixedBufPool;
/// use retty_io::buf::IoBuf;
/// use std::iter;
/// use std::mem;
///
...
impl<T: IoBufMut> FixedBufPool<T> {
    ...
    ///
    /// This method must be called in the context of a `retty-io` runtime.
    ...
    /// of the `retty-io` runtime this call is made in, the function returns
    /// an error.
